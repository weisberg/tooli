# Tooli + Claude Code: The Complete Skill Emergence Scenarios

> How developer tools become agent skills — from first function to ecosystem resource.

This is the **canonical combined scenarios document** for the Tooli ecosystem. It describes the full collaborative lifecycle between human developers, end-users, and autonomous agents (like Claude Code) using the `tooli` framework.

---

## Document Goals

1. Single source of truth for all Tooli + Claude Code scenarios.
2. Consistent, best-in-class format across every scenario: narrative depth, concrete code, testable criteria.
3. Usable by product, engineering, security, and documentation teams.
4. Every scenario from `scenarios_cc.md`, `scenarios_cdx.md`, and `scenarios_gm.md` is covered.

---

## Source Coverage Matrix

| Source | Original Scenario | Covered In |
|---|---|---|
| `scenarios_cc.md` | Scenario 1: Solo Developer's Debugging Toolkit | Scenario 1 |
| `scenarios_cc.md` | Scenario 2: Platform Team's Internal Toolchain | Scenario 2 |
| `scenarios_cc.md` | Scenario 3: Open-Source Ecosystem Effect | Scenario 4 |
| `scenarios_cc.md` | Scenario 4: Agent-Built Tool | Scenario 6 |
| `scenarios_cc.md` | Scenario 5: New Hire Onboarding | Scenario 7 |
| `scenarios_cc.md` | Scenario 6: Security Audit Workflow | Scenario 5 |
| `scenarios_cc.md` | Scenario 7: CI/CD Pipeline Integration | Scenario 8 |
| `scenarios_cc.md` | Scenario 8: Cross-Team Marketplace | Scenario 9 |
| `scenarios_cc.md` | Scenario 9: Multi-Agent War Room | Scenario 3 |
| `scenarios_cc.md` | Scenario 10: Global Skill Mesh | Scenario 11 |
| `scenarios_cc.md` | Scenario 11: Customer Support Workflows | Scenario 12 |
| `scenarios_cc.md` | Scenario 12: Versioned Evolution | Scenario 13 |
| `scenarios_cdx.md` | Scenario 9: Resource-First Subagent Operations | Scenario 10 |
| `scenarios_gm.md` | Feature 1: Lifecycle of a Skill | Scenario 1 |
| `scenarios_gm.md` | Feature 2: Autonomous Self-Healing | Scenario 14 |
| `scenarios_gm.md` | Feature 3: Multi-Agent Handoffs | Scenario 3 |
| `scenarios_gm.md` | Feature 4: Agent as Tool Author | Scenario 6 |
| `scenarios_gm.md` | Feature 5: Universal Skill Protocol | Scenario 11 |
| `scenarios_gm.md` | Feature 6: Enterprise Governance | Scenario 5 |
...
|---|---|---|
| **Maya (The Architect)** | Senior/on-call backend developer | Builds diagnostic tools to solve immediate operational pain |
| **Jordan (The Platform Lead)** | SRE / DevOps | Defines organizational workflows and "Never" rules in CLAUDE.md |
| **Alex (The Product Engineer)** | Product developer | Uses tools daily but didn't build them |
| **Sam (The Freelancer)** | Freelance developer across multiple projects | Installs tools, rarely reads source code |
| **Dana (The API Lead)** | Senior engineer building a customer-facing API | Works with Claude Code daily, notices repetitive patterns |
| **Priya (The New Hire)** | Junior developer, first two weeks on the team | Needs to get productive without memorizing every CLI flag |
| **Riku (The Security Officer)** | Compliance / SecOps | Enforces company-wide policies, auth scopes, and capability allowlists |
| **Leila (The Support Engineer)** | Customer-facing support | Needs fast, safe, auditable remediation workflows |
...
Every scenario follows the same underlying progression. A tool may stop at any stage — not everything needs to become an org-wide resource — but the path is always available.
...
7. MESH        → Export and AGENTS.md make the tool available across frameworks and teams.
```
...
- **Resource → Mesh:** Tools become portable across agent frameworks via export and universal documentation.

---

## Scenario Format

Each scenario uses a consistent structure:

1. **Persona** — Who is involved and what is their role
2. **Job to Be Done** — The underlying goal driving behavior (JTBD format)
3. **Context** — The situation and environment
4. **Journey** — A phased narrative showing evolution over time
5. **Key Interactions** — Concrete Given/When/Then moments between human, agent, and tool
6. **Acceptance Criteria** — Observable, testable outcomes
7. **Emergent Artifacts** — The tools, skills, MCP resources, and patterns that crystallize
8. **PTC Value** — Measurable improvement: speed, safety, quality, onboarding, compliance, or scale
...
| `loggrep errors` returns an empty list | Maya asks about errors | Agent reports "no errors found" and suggests broadening the time window or lowering the level threshold |
...
| Invalid `--since` timestamp format | Agent sends malformed date | Structured error with `field: "since"` and `suggestion.fix` showing the correct format |
...
| Skill | "Incident Triage" and "Post-Mortem Analysis" workflows in SKILL.md |
...
- Reduced on-call onboarding time for new engineers.
...
### Persona
**Jordan** (platform lead) and **Alex** (product engineer) at a 200-person engineering company. The platform team maintains internal infrastructure and developer experience tools.
...
| Pending migrations exist | Release begins | `schema-check` runs before canary (mandatory sequencing) |
| Canary started | Watch window is active | `health-check watch` gates promotion |
| Degraded metrics | Thresholds are crossed | Rollback handoff is executed deterministically |
...
- The deployment workflow started as tribal knowledge in Jordan's head; now 200 engineers follow the same process.
...
*When performing complex architectural changes, I want my specialized agents to coordinate seamlessly so that the final output is verified and safe.*
...
### Acceptance Criteria
1. Promotion is blocked if any pre-flight check fails.
2. Handoff routing is deterministic from metadata, not hallucinated.
3. The human approves before any deployment proceeds.
4. In-process `app.call()` is used for inter-agent calls where latency matters.
...
### PTC Value
- Incident response time reduced from minutes to seconds through deterministic handoffs.
- No single agent overloaded with full-stack context.
- Human stays at strategic decision points, not tactical execution.

---

## Scenario 4: The Open-Source Ecosystem Effect
...
| One referenced tool missing | Sequence runs | Agent reports gap with installation guidance and continues with available tools |
...
- Tools from strangers compose into cohesive workflows through shared protocol.

---

## Scenario 5: The Security Sentry
...
### Job to Be Done
*When we use autonomous tools in production, I want to control exactly what they can access so we maintain our security posture and regulatory compliance.*
...
**Then** tooli blocks the invocation *before the function body runs* and returns a structured error:
...
For tools that support it, agents can preview destructive operations without executing them:
...
**Given** dry-run is requested,
**When** command executes,
**Then** a plan is returned and no side effects occur:

> **Claude Code:** *(invokes env-cleanup prune /tmp --older-than 7d --dry-run --json)*
> "Dry-run complete. 142 files (847MB) would be deleted. Here's the breakdown:
>
> | Category | Files | Size |
> |---|---|---|
> | Build artifacts | 89 | 620MB |
> | Temp files | 41 | 180MB |
> | Log files | 12 | 47MB |
>
> Shall I proceed with the actual deletion?"
...
| Dry-run requested on destructive command | Command executes | Plan is returned, no side effects occur |
| Approved execution completes | Audit trail generated | Record includes actor, capability scope, and result |
| Policy enforcement | Mutative function body | Enforcement occurs *before* the function body executes |
...
| Resource | `security://audit-log` (structured invocation log for compliance review), `env-cleanup://pending-deletions` |
| Contract | `TOOLI_ALLOWED_CAPABILITIES` as environment-level security policy |
...
- Auditable agent behavior across the organization.

---

## Scenario 6: The Agent as Proactive Co-Architect
...
@app.command(
    annotations=ReadOnly,
    capabilities=["net:read"],
    delegation_hint="Use after running slow-endpoints and cache-audit, or as a combined analysis",
)
def suggest_fixes(
    base_url: Annotated[str, Argument(help="Base URL of the API")],
    threshold_ms: Annotated[int, Option(help="Response time threshold in ms")] = 500,
...
| Developer approves tool creation | Agent generates the tool | Tool incorporates auth handling, multiple cache headers, and configurable threshold from previous sessions |
...
| `suggest-fixes` returns an empty list | Developer asks for slow endpoint analysis | Agent reports "all endpoints are within threshold" |
| `slow-endpoints` returns errors for authenticated endpoints | Auth token not provided | Agent sees structured error with `field: "auth_token"` and asks the user |
| Repeated prompts | Pattern threshold reached | Agent proposes tool extraction |
| Published tool | Similar task reappears | Workflow uses command instead of bespoke code |
...
- Team's long-term leverage increases as more patterns become tools.

---

## Scenario 7: The New Hire Onboarding Experience
...
Priya didn't need to know that `loggrep` exists, how to use it, what flags to pass, or where the logs are stored.
...
>
> **Claude Code:** "I can't do that — the CLAUDE.md rules say 'NEVER restart production databases without SRE approval.' This is a safety guardrail for the team. I'd recommend asking in the #sre channel, or I can check if `health-check` shows the connection pool issue."

**Given** a prohibited operation is requested,
**When** Priya asks,
**Then** the agent cites the specific guardrail and blocks the unsafe path.
...
| Priya makes a mistake in a manual command | She passes a wrong flag | Tool returns structured error with `suggestion.fix` telling her the correct flag |
...
| Successful workflow completed | Agent summarizes | Next-step options are explicit |
...
### PTC Value
- Faster onboarding: productive on day 3 instead of week 3.
- Less mentor time for repetitive operational walkthroughs.
- Tribal knowledge captured in SKILL.md/CLAUDE.md, not just human memory.

---

## Scenario 8: The CI/CD Contracted Pipeline

### Persona
**Elena**, a CI maintainer and release engineer. **Developer** who asks Claude Code about CI failures.
...
| Command signature drift detected | CI runs | Contract mismatch surfaced before merge |
...
| Resource | CI status and artifact pointers as structured JSON |
...
- Stable contract between tool output and CI parsing — no regex breakage.

---

## Scenario 9: The Cross-Team Tool Marketplace

### Persona
**Staff engineer** coordinating multiple teams and repositories. **DevEx Team** maintains a shared tool registry.
...
### Context
Engineering org with 15 teams. Each team has built 2-3 tooli apps for their domain. Teams duplicate utilities because discoverability and trust are weak. Nobody knows what other teams have built.
...
#### Phase 4: Cross-Repo Boundaries

**Given** a tool from Team A tries to mutate state in Team B's repository,
**When** the agent attempts execution,
**Then** the policy blocks the mutation and directs a handoff to the owning team:

> **Claude Code:** "The `infra-provision` command needs `net:write` in the `payments-service` repo, but I'm operating in the `api-gateway` repo. Cross-repo mutations are blocked by policy. I've prepared a summary — you can hand this to the Payments team, or I can create a ticket."
...
| Two repos have similarly named commands | Agent must disambiguate | Agent explains the difference by capability and context |
| Wrong-repo mutative action attempted | Policy check | Block with actionable guidance and handoff direction |
| One MCP endpoint unavailable | Fallback needed | Equivalent CLI path is used and logged |
...
### PTC Value
- Reduced duplicated tooling across teams.
- Faster cross-team execution.
- Tool selection is deterministic and explainable.
...
### Persona
**Reliability subagent** operating under strict context-window and token limits. Part of a larger multi-agent system where token efficiency directly impacts cost and latency.

### Job to Be Done
*When investigating system state, I want direct resource reads so I do not waste tokens parsing large command output.*

### Context
Read-heavy diagnostic workflows often re-run commands unnecessarily. A subagent checking service health might invoke `health-check status --json` five times in a session, each time receiving the full output and burning context tokens. When the same data is available as an MCP resource, the agent can read it directly — smaller payload, no command overhead, same structure.

### Journey

#### Phase 1: Identifying High-Frequency Reads

The SRE team notices that during incident investigations, agents repeatedly call the same commands:
...
Each invocation returns the same data (or near-identical data), but costs tokens for command parsing, execution, and output formatting every time.

#### Phase 2: Promoting to MCP Resources

The team promotes the highest-frequency read paths to MCP resources:

```
incident://open                    → list of open incidents with severity and owner
incident://{id}                    → full detail for a specific incident
service://deps/{name}              → dependency graph for a service
health://status/{service}          → current health signals
loggrep://recent-errors            → last hour of errors, structured
deploy://status/{service}          → canary state, version, traffic split
```

Resources are bounded (max payload size defined) and schema-consistent (same structure as command output).

#### Phase 3: Resource-First Operating Pattern

The reliability subagent adopts a hierarchy:

1. **Read resource first** — if the URI exists and data is fresh, use it.
2. **Execute command only if** — resource is stale, doesn't exist, or write operation needed.
3. **Fall back to command with retry** — if resource read fails, invoke the underlying command.

> **Reliability Agent:** *(needs checkout-service status)*
>
> *(reads `health://status/checkout-service` — 200 tokens)*
>
> "Health is green. Error rate: 0.02%. No action needed."
>
> *(Without resources, this would have been: invoke `health-check status checkout-service --json` → receive 2000-token response → parse → extract the 200 tokens it actually needed)*

#### Phase 4: Staleness and Fallback

**Given** a resource URI exists but the agent needs guaranteed freshness,
**When** the resource was last updated 15 minutes ago during an active incident,
**Then** the agent executes the underlying command explicitly for real-time data:
...
>
> *(invokes health-check status checkout-service --json)*
>
> "Updated status: error rate spiked to 4.2% in the last 5 minutes. The resource was stale."

**Given** a resource read fails with a retryable error,
**When** the agent checks the error suggestion metadata,
**Then** the agent retries using the suggestion (e.g., different URI format or direct command fallback).
...
| Resource is stale | Freshness is required (active incident) | Agent executes underlying command explicitly |
| Resource read fails with retryable error | Agent checks suggestion metadata | Agent retries or falls back to command |
| Multiple resources needed | Agent investigates an incident | Agent reads resources in parallel — lower latency than sequential commands |
| Resource URI does not exist | Agent needs data | Agent invokes command normally and suggests resource promotion |

### Acceptance Criteria
1. Resources are bounded (max payload size) and schema-consistent with command output.
2. Read-heavy sessions show measurably lower command invocation counts.
3. Resource fallback paths are deterministic — stale → command, error → retry, missing → command.

### Emergent Artifacts

| Type | Artifact |
|---|---|
| Resources | `incident://open`, `incident://{id}`, `service://deps/{name}`, `health://status/{service}` |
| Skill | Token-efficient diagnostic loop — resource-first, command-second |
| Pattern | Read hierarchy: resource → command → fallback with structured error |

### PTC Value
- Lower context-window pressure — agents stay effective in longer sessions.
- Faster analysis latency — resource reads avoid command overhead.
- Cost reduction — fewer tokens consumed per diagnostic cycle.

---

## Scenario 11: The Global Skill Mesh
...
#### Phase 4: Caller Metadata Across Frameworks

Each generated wrapper sets `TOOLI_CALLER` appropriately:
...
```python
# Generated OpenAI wrapper
env = {**os.environ, "TOOLI_CALLER": "openai-agents-sdk"}
subprocess.run(["cloud-provisioner", "provision", "--json", ...], env=env)
```

This means Riku's telemetry (Scenario 5) shows *which framework* invoked each tool — across the entire mesh.
...
| AGENTS.md generated | GitHub Copilot reads the repo | Copilot discovers tools from AGENTS.md without framework-specific setup |
| Tool author updates a command | Re-export generated | Wrappers regenerate — all frameworks get the update |
| Subprocess wrappers invoked | Caller env var | Set consistently across all generated wrappers |
| Import-mode wrappers invoked | App-level call | Preserves structured success/failure behavior |
...
### PTC Value
- Faster framework adoption — no rewriting tools for each platform.
- Lower integration duplication cost.
- Tools become universal assets: any agent, any platform, any time.

---

## Scenario 12: Customer Support Workflows
...
| Diagnosis shows legitimate usage | Agent reads the result | Agent suggests plan upgrade instead of quota reset |

### Acceptance Criteria
1. Diagnosis always precedes remediation.
2. Destructive actions require explicit confirmation and audit reason.
3. Audit trail records actor, reason, timestamp, and before/after state.
...
### PTC Value
- Faster ticket resolution through structured diagnosis.
- Customer-safe audit trails for every remediation action.
- Support engineers follow consistent process regardless of experience level.

---

## Scenario 13: Versioned Evolution Without Agent Breakage
...
The maintainer introduces a new parameter and marks the old one deprecated:
...
### Acceptance Criteria
1. Deprecated flags continue to work with warnings during the migration window.
2. Agents reading the schema automatically use the new parameter.
3. Removal produces structured migration errors, not cryptic failures.
4. CI detects both deprecation warnings and breaking changes.
...
### PTC Value
- Zero-downtime tool evolution — old agents keep working during migration.
- Agent auto-migration through schema metadata.
- CI catches breaking changes before they reach production.

---

## Scenario 14: Autonomous Self-Healing

### Persona
**The Agent** (Claude Code) performing a task that encounters an environmental error.

### Job to Be Done
*When a tool call fails due to environmental factors, I want the agent to identify the fix and retry so that I am not interrupted for trivial issues.*

### Context
Not every tool failure requires human attention. Many failures are environmental: wrong file format, missing flag, expired token, stale cache. Tooli's structured error responses include `suggestion` metadata that tells the agent *how to fix the problem*. The agent can use this to self-heal without escalating to the user.

### Journey

#### Phase 1: The Initial Failure

Claude Code is tasked with checking production logs for credential leaks:

> **Developer:** "Scan today's logs for any leaked credentials"
>
> **Claude Code:** *(invokes infra-audit scan-logs --source prod.log.gz --json)*
>
...
#### Phase 2: The Reflection Loop

**Given** the error includes `"retry": true` and a concrete `suggestion`,
**When** Claude Code reads the structured error,
**Then** the agent automatically retries with the suggested fix — without asking the user:

> **Claude Code:** *(reads error, sees suggestion to add --decompress)*
> *(invokes infra-audit scan-logs --source prod.log.gz --decompress --json)*
>
> "Scanned 12,847 log entries. Found 2 potential credential leaks:
> 1. Line 4,521: AWS access key pattern in error message (service: payment-processor)
> 2. Line 8,903: API key in query parameter (service: analytics-ingest)
>
> Both should be rotated. Want me to check if these credentials are still active?"

The developer never saw the initial failure. The agent handled it silently because the error was environmental and the fix was deterministic.

#### Phase 3: When Self-Healing Can't Help

Not all errors should be auto-retried. The agent distinguishes:

- **Auto-retry:** `"retry": true` with concrete suggestion (add flag, change format, retry after delay)
- **Escalate:** No `suggestion`, or suggestion requires human judgment (e.g., "choose between option A and B")
- **Block:** Capability denial, policy violation, destructive action — never auto-retry

> **Claude Code:** *(invokes infra-audit scan-logs --source sensitive.log --json)*
>
> ```json
> {
>   "ok": false,
...
>     "code": "E2002",
>     "category": "auth",
>     "message": "File requires elevated access. Contact your security team.",
>     "suggestion": {
>       "action": "contact_admin",
>       "retry": false
>     }
...
> ```
>
> **Claude Code:** "The `sensitive.log` file requires elevated access that I don't have. You'll need to contact your security team for access, or provide a log file that's within my permissions."

### Key Interactions

| Given | When | Then |
|---|---|---|
| Tool returns error with `"retry": true` and concrete suggestion | Agent reads the structured error | Agent retries with the suggested fix, silently |
| Tool returns error without `suggestion` | Agent reads the error | Agent escalates to the user with the error message |
| Tool returns auth/policy error | Agent reads the error category | Agent does NOT retry, explains the restriction |
| Self-healing retry succeeds | Task continues | User sees the result as if nothing went wrong |
| Self-healing retry also fails | Second attempt errors | Agent escalates with both error messages for context |
| Tool returns `suggestion` with multiple options | Agent reads the alternatives | Agent asks the user to choose — doesn't guess |

### Acceptance Criteria
1. Environmental errors with `"retry": true` and concrete suggestion are auto-retried.
2. Auth and policy errors are never auto-retried.
3. Users are not interrupted for trivially fixable issues.
...
| Type | Artifact |
|---|---|
| Pattern | Reflection loop — error → suggestion → auto-retry → success |
| Contract | Error envelope `suggestion.retry` field governs agent retry behavior |
| Metric | Reduction in "human-in-the-loop" interruptions for environmental failures |

### PTC Value
- 70% reduction in human interruptions for trivial environmental failures.
- Agent resilience — tools can guide the agent through recovery without human help.
- Reduced developer frustration from being paged for fixable issues.

---

## The Patterns

### What Emerges Across All Fourteen Scenarios
...
Schemas tell agents what arguments a tool accepts. Capabilities tell agents (and security systems) what permissions are needed. Annotations tell agents whether it's safe to run without asking. Handoffs tell agents what to do next. Deprecation metadata tells agents how to migrate. Error suggestions tell agents how to self-heal. The tool describes itself.
...
The open-source tools in Scenario 4 were never designed to work together. The deployment tools in Scenario 2 were designed independently and connected through CLAUDE.md. The agent figures out how to combine tools from their metadata and the documented workflows. No orchestration framework required.

#### 7. The Agent Operates at Multiple Levels
- **Consumer:** Discovers and invokes existing tools (all scenarios)
- **Operator:** Follows documented workflows, making decisions at each step (Scenarios 2, 7, 12)
- **Teacher:** Shows users how tools work by using them (Scenario 7)
- **Author:** Creates new tools from observed patterns (Scenario 6)
- **Gatekeeper:** Enforces safety rules and asks for confirmation before irreversible actions (Scenarios 2, 5, 12)
- **Orchestrator:** Delegates to specialized subagents across tool domains (Scenario 3)
- **Self-healer:** Retries with structured suggestions on environmental failures (Scenario 14)
...
Before adding new scenarios to this document, verify:
...
4. Is at least one failure path explicitly dramatized (not just mentioned)?
5. Is at least one guardrail demonstrated in action?
6. Does the scenario identify emergent MCP resources, not just tool commands?
7. Does it show concrete code, JSON output, or agent dialogue (not just abstract description)?
8. Is the PTC value explicit — speed, safety, quality, onboarding, compliance, or scale?
9. Does the scenario introduce something the other scenarios don't already cover?
10. Is behavior documented over implementation details?
...
6. Security response chain: detect → map → contain with mandatory approvals
