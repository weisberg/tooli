...
class SkillV4Generator:
    ...
    def _critical_rules(self) -> list[str]:
        ...
        lines.extend([
            "## Agent Integration",
            "",
            "Set `TOOLI_CALLER` before invoking any command to identify your agent:",
            "",
            "```bash",
            f"TOOLI_CALLER=claude-code {self._tool_name} <command> --json",
            "```",
            "",
            "| Variable | Required | Description |",
            "|---|---|---|",
            "| `TOOLI_CALLER` | Recommended | Agent identifier (e.g. `claude-code`, `cursor`, `aider`) |",
            "| `TOOLI_CALLER_VERSION` | No | Semver of your agent (e.g. `1.4.2`) |",
            "| `TOOLI_SESSION_ID` | No | Opaque session/run ID for tracing |",
            "",
            "When `TOOLI_CALLER` is set, the tool skips heuristic detection and immediately",
            "enters agent mode with 100% confidence. The `caller_id`, `caller_version`, and",
            "`session_id` fields appear in the JSON envelope `meta` block for tracing.",
            "",
            f"Use `{self._tool_name} detect-context --json` to inspect the current detection result.",
            "",
        ])
